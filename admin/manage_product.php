...
if(isset($_POST['submit'])){
    ...
    $meta_tiitle = get_safe_value($conn, $_POST['meta_tiitle']);
    ...
    if($msg ==''){
        if(isset($_GET['id'])&& $_GET['id']!=""){
            if($_FILES['image']['name']!=''){
                ...
                description='$description',meta_tiitle='$meta_tittle',meta_desc='$meta_desc',meta_keyword='$meta_keyword',image='$image' WHERE id ='$id'";
            }else{
                $update_sql = "UPDATE product SET categories_id='$categories_id',name='$name',mrp='$mrp',price='$price',qty='$qty',short_desc='$short_desc',
                description='$description',meta_tiitle='$meta_tittle',meta_desc='$meta_desc',meta_keyword='$meta_keyword' WHERE id ='$id'";
            }
            mysqli_query($conn, $update_sql);
        }else{
            $image =rand(111111111,999999999).'_'.$_FILES['image']['name'];
            move_uploaded_file($_FILES['image']['tmp_name'],PRODUCT_IMAGE_SERVER_PATH.$image);
            mysqli_query($conn, "INSERT INTO product(categories_id,name,mrp, price, qty, short_desc, description, meta_tiitle, meta_desc, meta_keyword, status,image) 
            VALUES ('$categories_id','$name','$mrp', '$price', '$qty', '$short_desc', '$description', '$meta_tiitle', '$meta_desc', '$meta_keyword', '1','$image')");
        }  
        header("location: product.php");
        die();
    }
                ...
                <form method="POST" enctype="multipart/form-data">
                    <div class="card-body card-block">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description" placeholder="Enter product short descriptin" class="form-control" required><?php echo $short_desc?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="name" class=" form-control-label">description</label>
                            <textarea name="short_desc" placeholder="Enter product descriptin" class="form-control" required><?php echo $description?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="name" class=" form-control-label">meta title</label>
                            <textarea name="meta_tiitle" placeholder="Enter product meta_title" class="form-control"><?php echo $meta_tiitle?></textarea>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="meta_keyword" placeholder="Enter product meta_title" class="form-control"><?php echo $meta_keyword?></textarea>
                        </div>
